# Project# Progress - 2026-02-28

- **뉴스 피드 안정화 및 고도화 (Yahoo Finance RSS)**:
    - 외부 프록시 서버 의존성을 제거하고 직접 구현한 백엔드 API(`/api/news`)를 통해 야후 파이낸스 RSS 피드를 연동했습니다.
    - 관심 종목(티커)별 맞춤 뉴스 수집 로직을 구글 뉴스에서 야후 파이낸스 직결 방식으로 전환하여 데이터 신뢰도를 높였습니다.
    - 뉴스 발행 시간의 KST 동기화 및 가독성 높은 상대 시간(방금 전, n분 전 등) 포맷팅을 적용했습니다.
