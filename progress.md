...
## [2026-03-01 12:10] V2 모바일 자산 등록 기능 추가 및 종목 통합 로직 구현

- **D 번 카드(주식 목록) 자산 추가 기능 구현**:
    - **자산 등록 버튼 추가**: `AssetListGroupCard` 하단에 V2 스타일의 점선 테두리 '+' 버튼을 추가하여 접근성을 높였습니다.
    - **V2 자산 등록 시트(AssetEntrySheetV2) 신설**: V1의 `StockEntryForm`을 그대로 활용하면서도 V2의 프리미엄 디자인(92vh 높이, 라운드 코너, 다크 모드 지원)을 적용한 전용 시트를 구축했습니다.
- **지능형 종목 통합(Merging) 및 가중 평균 계산**:
    - **실시간 종목 병합**: 동일한 티커 심볼을 가진 자산들을 `SimpleModeV2Container`에서 자동으로 병합하여 하나의 카드로 표시되도록 개선했습니다.
    - **가중 평균 평단가 산출**: 여러 계좌에서 매수한 경우에도 전체 수량 대비 총 매수 금액을 계산하여 정확한 통합 평단가를 제공합니다.
- **상세 매수 히스토리 및 개별 수익률 관리**:
    - **개별 이력 리스팅**: 종목 상세 시트 하단에 통합된 모든 매수 데이터를 개별 행으로 나열하여 투명성을 확보했습니다.
    - **시점별 수익률 계산**: 각 매수 건별 평단가와 현재 시장가를 실시간 비교하여, 개별 거래의 성과(손익 금액 및 수익률%)를 직관적으로 확인할 수 있도록 고도화했습니다.
- **UI 안정성 및 린트 오류 해결**:
    - 중복 변수 선언 제거 및 누락된 컴포넌트 임포트 정정을 통해 빌드 안정성을 확보했습니다.
