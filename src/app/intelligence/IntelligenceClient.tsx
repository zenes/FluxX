...
export default function IntelligenceClient({ initialData }: { initialData: any }) {
    ...
    const nodes = useMemo(() => {
        ...
        // Core size - Significantly larger
        const coreSize = 200;
        ...
        // Base sizing logic for asset bubbles - Larger range
        ...
        // 2. Organic Layout: Force Relaxation with Boundary Constraints
        const iterations = 80;
        const padding = 20;
        ...
        for (let i = 0; i < iterations; i++) {
            for (let j = 0; j < allNodes.length; j++) {
                ...
                // Centripetal Force: Slight pull towards center to keep things packed
                if (n1.type !== 'core') {
                ...
                // Node-on-Node Collision/Repulsion
                for (let k = j + 1; k < allNodes.length; k++) {
                    ...
                    const minDistance = (n1.size + n2.size) / 2 + 8; // Tighter padding for organic feel
    ...
    return (
        <div className="w-full h-[750px] flex flex-col gap-4">
            <Card className="flex-1 border-primary/20 bg-background/50 backdrop-blur-sm shadow-2xl overflow-hidden relative" ref={containerRef}>
                {/* Theme-aware background with subtle grid */}
                ...
                <CardContent className="h-full w-full relative p-0 overflow-hidden">
                    <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
                        ...
                        {nodes?.children?.map(acc => {
                            const isAccRelated = selectedNode === acc.id || acc.children?.some(as => as.id === selectedNode) || hoveredNode === acc.id || acc.children?.some(as => as.id === hoveredNode);

                            return (
                                <g key={acc.id}>
                                    {/* Branches from Core to Accounts */}
                                    <path
                                    ...
                                    {acc.children?.map(asset => {
                                        const isAssetRelated = selectedNode === asset.id || hoveredNode === asset.id || (selectedNode === acc.id) || (hoveredNode === acc.id);

                                        return (
                                            /* Branches from Accounts to Assets */
                                            <path
                                                key={asset.id}
                    ...
                    {nodes && (
                        <>
                            {/* The Core: Theme Aware */}
                            <div
                                className="absolute bg-background rounded-full flex flex-col items-center justify-center text-primary z-40 transition-all duration-700 cursor-pointer border-2 border-primary shadow-xl hover:shadow-primary/30 group active:scale-95"
                                style={{
                            ...
                            >
                                ...
                                <span className="text-2xl font-black tracking-tight text-foreground transition-all duration-300 group-hover:scale-110">${initialData.totalNetWorth.toLocaleString()}</span>
                                <div className="mt-1 size-1.5 bg-primary rounded-full animate-ping opacity-50" />
                            </div>

                            {nodes.children?.map(acc => (
                                <div key={acc.id}>
                                    {/* Account Nodes: Theme Aware Glassmorphism */}
                                    <div
                                        className={`absolute bg-background/80 backdrop-blur-xl border-2 rounded-full flex items-center justify-center z-30 transition-all duration-500 cursor-pointer group hover:shadow-lg active:scale-90 ${selectedNode === acc.id ? 'border-primary shadow-primary/30' : 'border-primary/30 hover:border-primary hover:shadow-primary/20'}`}
                                        style={{
                                    ...
                                    >
                                        ...
                                        {/* Floating Label */}
                                        <div className={`absolute -top-12 left-1/2 -translate-x-1/2 bg-popover border border-border px-3 py-1.5 rounded shadow-2xl z-50 transition-all duration-300 pointer-events-none ${hoveredNode === acc.id ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-90'}`}>
                                    ...
                                    {acc.children?.map(asset => (
                                        <div
                                            key={asset.id}
                                            className={`absolute z-20 transition-all duration-500 hover:scale-125 hover:z-50 cursor-pointer group active:scale-95`}
                                            style={{
                                        ...
                                        >
                                            <div className={`relative w-full h-full rounded-full overflow-hidden border-2 bg-background transition-all duration-300 shadow-md flex items-center justify-center ${selectedNode === asset.id ? 'border-primary ring-4 ring-primary/20' : 'border-border group-hover:border-primary'}`}>
                                            ...
                                            <div className={`absolute -bottom-10 left-1/2 -translate-x-1/2 bg-popover/90 backdrop-blur-md border border-border px-2 py-1 rounded text-[10px] z-50 transition-all duration-300 pointer-events-none ${hoveredNode === asset.id ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
                            ...
                            {ripple && (
                                <div
                                    className="absolute rounded-full border-2 border-primary pointer-events-none z-50 animate-ripple"
                                    style={{
                                        left: ripple.x,
                                        top: ripple.y,
                                        width: 10,
            ...
            {/* Bottom Status Bar: Theme Aware */}
            <div className="flex gap-4 h-24 font-mono">
                <Card className="flex-[2] border-primary/10 bg-muted/20 flex items-center px-6 relative overflow-hidden group">
                    ...
                    <div className="space-y-1 relative z-10 w-full">
                        ...
                        <div className="text-xs text-foreground/80 leading-relaxed overflow-hidden whitespace-nowrap overflow-ellipsis">
